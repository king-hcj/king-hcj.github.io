...
&emsp;&emsp;因业务需要CRON生成器，网上找的插件多不可用，[在线cron工具](http://cron.qqe2.com/){:target='_blank'}又没法直接集成到项目中，遂参考在线工具，使用react和antd实现一个CRON表达式生成器，并在接口的配合下完成合规校验。

效果图如下：<br/>

![示例图片]({{site.url}}{{site.baseurl}}/images/posts/javascript/cron.png?raw=true)
...
>>>错误解决：{ {second: *}本来前两个花括号之间没有空格({\{second: *})，导致报错‘Variable '{\{second: *}' was not properly terminated with regexp: /\}\}/ in...’，可以用转译，也可以用空格隔开。也就是说不能直接不现两个前花括号连着的情况。

后期优化参考：
- [Cron 表达式的JavaScript验证代码](https://blog.csdn.net/zsg88/article/details/70622414){:target='_blank'}