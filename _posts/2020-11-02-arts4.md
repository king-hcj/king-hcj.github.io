...
- Tip：
  ...
  - mac突然连不上Wi-Fi：排除密码、DNS等情况外，可以考虑是不是外接设备的问题。第一个方法，拔掉你的外接type-c就可以了(不是电源线，是你扩展的type-c接口，用来连接硬盘或者手机时候用的)； 第二种，更换一个5GHz的路由器。因为将USB2.0的接收器连接在USB3.0端口附近的插槽就会导致无线接收器信号被干扰。
