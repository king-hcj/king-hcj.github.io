...
- Tip：
  ...
  - [echarts地图](https://echarts.apache.org/examples/zh/editor.html?c=effectScatter-bmap){:target='_blank'}
  - [echarts-for-react遇坑](https://www.cnblogs.com/cazj/p/11812697.html){:target='_blank'}：series数据之间有空时就不会连接，会出现断点（解决方案：在series的子对象中加入`connectNulls：true`即可解决）；监听点击图例事件；
  - 正则表达式-从url里截取文件名：`'http://qcrm.aaa.Dfiless.com/salary/1600939131210_aW1wb3J0.xlsx'.match(/(?<=\d{13}_).*(\.doc|\.docx|\.pdf|\.xls|\.xlxs)$/)?.[0]`，输出`aW1wb3J0.xlsx`
