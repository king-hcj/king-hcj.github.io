...
- Tip：
  ...
  - 修改 url 不刷新页面（pushState 与 replaceState）
    - history.pushState(state,title,url)：
      ...
      ```
      > [前端路由实现：通过 onpopstate()监控浏览器前进后退数据](https://newsn.net/say/history-popstate.html){:target='\_blank'}
...
- Tip：
  ...
  ![divider]({{site.url}}{{site.baseurl}}/images/posts/arts/divider.png?raw=true)
...
- Tip：
  ...
  - 使用 loadsh 的`_.cloneDeep(obj)`时，要特别注意 obj 中是否包含 React node 节点，否则可能会造成栈溢出；
  ...
  - 谷歌 Chrome 浏览器出现 Whitelabel Error Page 或者不停请求现象的解决：打开`chrome://flags/`，禁用（设置为 Disabled）`SameSite by default cookies`和`Enable removing SameSite=None cookies`，重启浏览器（如果还是不行，可尝试禁用`Cookies without SameSite must be secure`）
    ![samesite.png]({{site.url}}{{site.baseurl}}/images/posts/arts/samesite.png?raw=true)

- Share：

  - [React 富文本编辑器（braft-editor）](https://braft.margox.cn/){:target='\_blank'}
  - [braft-extensions](https://github.com/margox/braft-extensions){:target='\_blank'}：Braft Editor 扩展模块包
